@chcp 65001 >nul
@echo off
REM build_windows.bat — Build Uni-Dock on Windows
REM
REM Prerequisites:
REM   Visual Studio 2019/2022 (MSVC) with "Desktop development with C++" workload
REM   CUDA Toolkit >= 11.8  from https://developer.nvidia.com/cuda-downloads
REM     (select Visual Studio integration during install)
REM   CMake >= 3.16         from https://cmake.org/download/
REM   Boost >= 1.72         from https://www.boost.org  OR use --fetch-boost
REM
REM Usage (run in "x64 Native Tools Command Prompt for VS"):
...
REM   build_windows.bat --fetch-boost  -- auto-download Boost

setlocal enabledelayedexpansion

set SCRIPT_DIR=%~dp0
set SOURCE_DIR=%SCRIPT_DIR%unidock
...
call :build_variant CPU ON OFF "%SOURCE_DIR%\build_cpu" Uni-Dock.exe
if errorlevel 1 exit /b 1

call :build_variant GPU OFF ON "%SOURCE_DIR%\build_gpu" Uni-Dock-GPU.exe
if errorlevel 1 exit /b 1
...
cmake -S "%SOURCE_DIR%" -B "%BUILD_DIR%" %CMAKE_GENERATOR_ARGS% %CUDA_TOOLSET_ARG% ^
...
copy /Y "%BUILD_DIR%\Release\unidock.exe" "%TARGET_EXE%" >nul
if errorlevel 1 (
  echo ERROR: Failed to copy output binary.
  exit /b 1
)

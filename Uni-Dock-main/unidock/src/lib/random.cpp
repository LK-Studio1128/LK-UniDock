...
fl random_fl(fl a, fl b, rng& generator) {  // expects a < b, returns rand in [a, b]
    assert(a < b);
    std::uniform_real_distribution<fl> distribution(a, std::nextafter(b, std::numeric_limits<fl>::max()));
    fl tmp = distribution(generator);
    assert(tmp >= a);
    assert(tmp <= b);
    return tmp;
}
...
fl random_normal(fl mean, fl sigma, rng& generator) {  // expects sigma >= 0
    assert(sigma >= 0);
    std::normal_distribution<fl> distribution(mean, sigma);
...
vec random_inside_sphere(rng& generator) {
    while (true) {  // on average, this will have to be run about twice
        ...
        vec tmp(r1, r2, r3);
        if (sqr(tmp) < 1 && tmp.norm() > 10 * epsilon_fl) return tmp;
    }
}

...
bug reporting, license agreements, and more information.      \n";
    ...
    try {
        ...
        // Use multiple workers for 1:1 docking, and exit (GPU only)
#ifdef ENABLE_CUDA
...
#endif // ENABLE_CUDA (paired_batch_size)
...
#else
            // CPU-only build: no GPU available, deviceCount stays 0
            (void)deviceCount; (void)avail; (void)total;
            printf("[CPU-only build] No GPU detected. Using system RAM for batch sizing.\n");
#endif
...
#pragma omp critical
                ...
                */
                ...
                template_batch_docking<MaxConfig>(v,all_ligands,maxGroup,"Max",exhaustiveness, multi_bias,max_memory,
                        ...
                        min_rmsd,max_evals,max_step,seed, refine_step, local_only, energy_range);
#else
                std::cerr << "[CPU-only build] GPU batch docking unavailable. Use --ligand / --batch for CPU mode.\n";
#endif
            }
        }
    }

...
constexpr float epsilon_fl_gpu = FLT_EPSILON;
constexpr float pi_gpu = 3.1415927f;

// GPU equivalents of AD42/macrocycle helpers from potentials.h
__device__ __forceinline__ fl smoothen_gpu(fl r, fl rij, fl smoothing) {
    smoothing *= 0.5f;
    if (r > rij + smoothing) return r - smoothing;
    if (r < rij - smoothing) return r + smoothing;
    return rij;
}
__device__ __forceinline__ fl ad4_hb_eps_gpu(sz t) {
    if (t < AD_TYPE_SIZE) return ad_type_property_gpu(t).hb_depth;
    return 0;
}
...
__device__ __forceinline__ fl ad4_vdw_eps_gpu(sz t) {
    if (t < AD_TYPE_SIZE) return ad_type_property_gpu(t).depth;
    return 0;
}
...
__device__ __forceinline__ fl smooth_div_gpu(fl x, fl y) {
    if (std::abs(x) < epsilon_fl_gpu) return 0;
    if (std::abs(y) < epsilon_fl_gpu)
...
__device__ __forceinline__ fl volume_gpu(sz a_ad, sz a_xs) {
    if (a_ad < AD_TYPE_SIZE)
        ...
        return 4.0f * pi_gpu / 3.0f * pow(xs_radius_gpu(a_xs), 3);
...
__device__ __forceinline__ fl ad4_vdw_eval(sz a_ad, sz b_ad, fl r, fl smoothing, fl cap,
    ...
    fl r6 = pow(r, 6);
    fl r12 = pow(r, 12);
    if (r12 > epsilon_fl_gpu && r6 > epsilon_fl_gpu)
...
__device__ __forceinline__ fl ad4_hb_eval(sz a_ad, sz b_ad, fl r, fl smoothing, fl cap, fl cutoff) {
    ...
    fl r10 = pow(r, 10);
    fl r12 = pow(r, 12);
    if (r12 > epsilon_fl_gpu && r10 > epsilon_fl_gpu)

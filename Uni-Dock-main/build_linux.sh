#!/usr/bin/env bash
# build_linux.sh — Build Uni-Dock on Linux
# Prerequisites:
#   NVIDIA GPU + CUDA Toolkit >= 11.8   (for GPU build)
#   CMake >= 3.16
#   GCC/G++ compatible with NVCC
#   Boost >= 1.72  OR use -DFETCH_BOOST=ON to auto-download
#
# Usage:
...
if [[ "${BUILD_CPU}" == "ON" ]]; then
  build_variant "CPU" ON OFF "${SOURCE_DIR}/build_cpu" "Uni-Dock"
fi

if [[ "${BUILD_GPU}" == "ON" ]]; then
  build_variant "GPU" OFF ON "${SOURCE_DIR}/build_gpu" "Uni-Dock-GPU"
fi

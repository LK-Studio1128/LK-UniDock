...
float get_total_system_memory_mb() {
#ifdef _WIN32
    MEMORYSTATUSEX status;
    status.dwLength = sizeof(status);
    if (GlobalMemoryStatusEx(&status)) {
        return static_cast<float>(status.ullTotalPhys / 1024.0 / 1024.0);
    }
    return 0.0f;
#else
    return static_cast<float>(sysconf(_SC_PHYS_PAGES))
           * static_cast<float>(sysconf(_SC_PAGE_SIZE)) / 1024.0f / 1024.0f;
#endif
}
...
#endif // ENABLE_CUDA (template_batch_docking)
...
bug reporting, license agreements, and more information.      \n";
    ...
    try {
        ...
        inputs.add_options()
            ("receptor", value<std::string>(&rigid_name),
            ...
            ("ligand", value<std::vector<std::string> >(&ligand_names)->multitoken(),
             "ligand (PDBQT)")
            ("ligand_index", value<std::string>(&ligand_index),
        ...
        advanced.add_options()
            ("device_id", value<int>(&device_id)->default_value(0),
            ...
            ("reference_ligand_scale", value<double>(&reference_ligand_scale)->default_value(reference_ligand_scale),
            ...
            ("weight_gauss2", value<double>(&weight_gauss2)->default_value(weight_gauss2),
             "gauss_2 weight")
            ("weight_repulsion", value<double>(&weight_repulsion)->default_value(weight_repulsion),
             "repulsion weight")
            ("weight_hydrophobic", value<double>(&weight_hydrophobic)->default_value(weight_hydrophobic),
             "hydrophobic weight")
            ...
            ("weight_vinardo_repulsion",
             value<double>(&weight_vinardo_repulsion)->default_value(weight_vinardo_repulsion),
             "Vinardo repulsion weight")
            ("weight_vinardo_hydrophobic",
             value<double>(&weight_vinardo_hydrophobic)->default_value(weight_vinardo_hydrophobic),
            ...
            ("weight_glue", value<double>(&weight_glue)->default_value(weight_glue),
             "macrocycle glue weight")
            ("keep_nonpolar_H", bool_switch(&keep_H)->default_value(keep_H),
        ...
        if (vm.count("ligand")) {
...
#else
                // CPU fallback: use Vina-style multi-threaded Monte Carlo search
            ...
            float max_memory = get_total_system_memory_mb() * 0.95f;
            bool use_v100 = true;
#ifdef ENABLE_CUDA
...
#else
            (void)deviceCount; (void)avail; (void)total;
            printf("[CPU-only build] No GPU. Using system RAM for batch sizing.\n");
#endif
...
#pragma omp critical
                ...
                */
                ...
                printf("max_num_atoms:%zu\n", max_num_atoms);
                printf("max_num_torsions:%zu\n", max_num_torsions);
                printf("max_num_rigids:%zu\n", max_num_rigids);
                printf("max_num_lig_pairs:%zu\n", max_num_lig_pairs);
                ...
                template_batch_docking<MaxConfig>(v,all_ligands,maxGroup,"Max",exhaustiveness, max_memory,
                        ...
                        min_rmsd,max_evals,max_step,seed, refine_step, local_only, energy_range);
#else
                // CPU fallback: process ligands one-by-one with CPU global_search
                printf("[CPU-only build] Using sequential CPU docking for %zu ligands.\n", all_ligands.size());
                for (size_t li = 0; li < all_ligands.size(); ++li) {
                    const auto& named_lig = all_ligands[li];
                    std::string lig_out = default_output(get_filename(named_lig.first), out_dir);
                    printf("Docking ligand %zu/%zu: %s\n", li + 1, all_ligands.size(),
                           get_filename(named_lig.first).c_str());
                    try {
                        Vina v1(v);
                        std::vector<model> ligs_cpu;
                        ligs_cpu.push_back(named_lig.second);
                        v1.set_ligand_from_object(ligs_cpu);
                        v1.global_search(exhaustiveness, num_modes, min_rmsd, max_evals);
                        v1.write_poses(lig_out, num_modes, energy_range);
                    } catch (std::exception& e) {
                        std::cerr << "WARNING: Failed to dock " << named_lig.first
                                  << ": " << e.what() << "\n";
                    }
                }
#endif

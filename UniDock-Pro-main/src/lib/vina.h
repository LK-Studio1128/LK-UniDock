...
public:
    // Constructor
    Vina(const std::string& sf_name = "vina", int seed = 0, int verbosity = 1,
        ...
        gpu = false;
        // Detect number of CPUs for CPU-based global search
        {
            unsigned num_cpus = boost::thread::hardware_concurrency();
            m_cpu = (num_cpus > 0) ? static_cast<int>(num_cpus) : 1;
        }
        ...
        if (sf_name.compare("vina") == 0) {
            ...
            std::cerr << "ERROR: Scoring function " << sf_name
                      << " not implemented (choices: vina or vinardo)\n";
            exit(EXIT_FAILURE);
        }
    }
    // Destructor
    ...
    // CPU-based global search (fallback when CUDA is not available)
    void global_search(const int exhaustiveness = 8, const int n_poses = 20,
                       const double min_rmsd = 1.0, const int max_evals = 0);
#ifdef ENABLE_CUDA
    // Note: Despite the name, this function runs on CPU but launches GPU kernels for the search
    // The GPU performs Monte Carlo search, while CPU handles pre/post-processing and refinement
...
#endif // ENABLE_CUDA (template global_search_gpu)
    ...
    triangular_matrix_cuda_t
        m_data_list_gpu[DefaultConfig::MAX_LIGAND_NUM];  // the pointer to precalculated output on GPU
#endif
...
#endif
    std::vector<double> calculate_score(model& mdl, const precalculate_byatom& prec,

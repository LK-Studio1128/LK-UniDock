#!/usr/bin/env bash
# build_mac.sh — Build UniDock-Pro on macOS (CPU-only)
#
# IMPORTANT: Apple Silicon (M1/M2/M3/M4) does NOT support NVIDIA CUDA.
#   GPU modes (classical docking acceleration, similarity search, hybrid docking)
#   are unavailable. This script builds a CPU-fallback binary for testing.
#
# Prerequisites:
...
echo "=== UniDock-Pro macOS Build (CPU-only) ==="
...
for dep in cmake; do
  if ! command -v $dep &>/dev/null; then
    echo "ERROR: $dep not found. Install via: brew install $dep"; exit 1
  fi
done

if ! brew list libomp &>/dev/null 2>&1; then
  echo "WARNING: libomp not found. Parallel batching will be disabled."
...
BIN_SRC="${BUILD_DIR}/udp"
BIN_DST="${DIST_DIR}/UniDock-Pro"
if [[ -f "${BIN_SRC}" ]]; then
  cp "${BIN_SRC}" "${BIN_DST}"
  chmod +x "${BIN_DST}"
fi
